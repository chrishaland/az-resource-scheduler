...
public class UpsertResourceHandler : CommandHandlerBase<UpsertResourceRequest>
{
    ...
    private async Task<(bool isBadRequest, string[] errorMessages)> IsBadRequest(UpsertResourceRequest request, CancellationToken ct)
    {
        ...
        var provider = await _context.Providers.SingleOrDefaultAsync(p => p.Id.Equals(request.ProviderId), ct);
        if (provider == null)
        {
            isBadRequest = true;
            errorMessages.Add("invalid_provider_id, must provide a valid provider for where the resource is located");
        }
